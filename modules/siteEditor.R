...
siteEditorController <- function(input, output, session) {
  ...
  observeEvent(input$title, {
    feedbackDanger(inputId=ns("title"), condition=(nchar(input$title) == 0), text="Investigator title cannot be blank")
  })
  
  observeEvent(input$givenName, {
    feedbackDanger(ns("givenName"), condition=(nchar(input$givenName) == 0), text="Investigator given name cannot be blank"
    )
  })
  
  observeEvent(input$familyName, {
    feedbackDanger(ns("familyName"), condition=(nchar(input$familyName) == 0), text="Investigator family name cannot be blank"
    )
  })
  
  observeEvent(input$emailAddress, {
    feedbackDanger(ns("emailAddress"), condition=!isValidEmailAddress(input$emailAddress), text="Please enter a valid email address")
  })
}