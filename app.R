...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (is.null(session$user)) {
      # Unauthorised usage
      if (session$clientData$url_hostname == "shinyapps.io") {
        session$userData$user <- list(SiteID="XXX", Admin=FALSE, EmailAddress="Unknown", Title="Unknown", GivenName="Unknown", FamilyName="Unknown")
        displayMessage("Error", HTML("User is undefined.  Unable to continue.<br>If you think this is an error, please contact the study administrators.<br><br>This dialog will close, and the application terminate, after 5 seconds."))
        Sys.sleep(5)
        shiny::stopApp()
      } else {
      ...
      # Production environment on shinyapps.io
      u <- credentials %>% filter(EmailAddress == tolower(session$user))
      if (nrow(u) != 1) {
        session$userData$user <- list(SiteID="XXX", Admin=FALSE, EmailAddress="Unknown")
        displayMessage("Error", HTML(paste0("Unable to locate credentials for user ", session$user, ".<br>If you think this is an error, please contact the study administrators.<br><br>This dialog will close, and the application terminate, after 5 seconds.")))
        Sys.sleep(5)
        shiny::stopApp()
      }
  ...
  observeEvent(input$age, {
    feedbackWarning(
      inputId="age",
      condition=input$age < 40 |  input$age > 90,
  ...
  observeEvent(input$icuAdmission, {
    feedbackDanger(
      "icuAdmission",
      condition=!isTruthy(input$icuAdmission),
  ...
  observeEvent(input$icuAdmissionAttempted, {
    feedbackDanger(
      ...
      condition=!isTruthy(input$icuAdmissionAttempted),
